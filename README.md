...
```
From the project root directory (`cd ..`), build the static frontend with Docker by running:

```shell
docker compose -f webapp/docker-compose.yml run --build webapp-dist
```

Still from the project root directory, deploy the frontend to NOUMENA Cloud with
